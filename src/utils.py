...
def get_scaled_data_new(data):
    ...
    features['f_ema_20'] = data['close'].ewm(span=20, adjust=False).mean().shift(1)
...
def prepare_aligned_data(ticker, config):
...
def statistics_calc(rets, bh, name = '_', save_path=Path(os.getcwd(), 'save', 'models'), plot = False, json_filename='stats.json', mode='train', ticker='none'):
    """
    ...
    # print(f'metrics: {sharpe}, {ret}, {vol}')
    if plot:
        ...
        print(f'Sharpe ratio: {sharpe}, annual return: {ret}, volatility: {vol}')
...
def daily_portfolio_return(env, model):
    obs = env.reset()
    done = [False]
    portfolio_values = []
    timestamps = []
    ...
    return daily['log_return'].dropna(), daily['log_return_b_h'].dropna()
